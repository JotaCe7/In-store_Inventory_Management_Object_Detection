...
if [ -n "${commit_sha}" ]; then
    ...
    mkdir -vp /home/app/.vscode-server/bin/"${commit_sha}"

    # Extract the tarball to the right location.
    tar --no-same-owner -xzv --strip-components=1 -C /home/app/.vscode-server/bin/"${commit_sha}" -f "/tmp/${archive}"
else
    echo "could not pre install vscode server"
fi

# export comand will not work in intermediate images
...
/home/app/.vscode-server/bin/"${commit_sha}"/bin/code-server --install-extension ms-python.python
